...
static unsigned char Set_palette = ZFALSE;	/* TRUE if EGA/VGA/MCGA card */
															...
															/* CGA white on black */
static unsigned char PS2 = ZFALSE;				/* Is this a PS/2? If so, int */
															/* 0x10, function 0xB is the */
															/* opposite of VGA/EGA usage */
...
/* Page 4-14 of the PS/2 & PC BIOS Technical Reference describes
   ...
   { /* Function supported, must have VGA Bios */
		Set_palette = ZTRUE;			/* show mucking needed for palette */
      s1 = regs.h.bl;
      if (s1 & 1) 
      ...
      if ( !(Display & (VID_CGA|VID_MCGA)) )
      { /* doesn't want CGA or MCGA, see if we MUST do MCGA */
			if ( PS2 )
			{ /* yup, default to MCGA on PS/2 */
				Display = VID_MCGA;
			}
			else
			{ /* on VGA/EGA, default to EGA */
				Display = VID_EGA;
			}
      }
		return;
...
void md_init_color()
{
   ...
   { /* just set up foreground color for CGA */
      ...
      Fgcolor = DEF_CGA_FG_COLOR;
		...
		if ( PS2 )
		{ /* set up the foreground color to be a dull white for the PS/2 */
			regs.h.ah = 0x0B;
			regs.h.bh = 0;
			regs.h.bl = 7;
			int86( 0x10, &regs, &regs );
		...
		{ /* set up the palette for white on black for VGA */
      	regs.h.ah = 0x0B;             /* Set Overscan Color */
      	regs.h.bh = 0;                /* Set Border Color */
			if ( PS2 )
			{ /* this is foreground color on MCGA */
				regs.h.bl = 0x07;				/* light white */
			}
			else
			{ /* make black be background/border color */
      		regs.h.bl = Bgcolor;          /* Color Value */
			}
      	int86(0x10, &regs, &regs);    /* Set BG for CGA */

			if ( !PS2 )
			{ /* make white be foreground color */
      		regs.h.ah = 0x10;             /* Set Palette Registers */
      		regs.h.al = 0x00;					/* Update Specific Register */
      		regs.h.bh = 0x07;					/* set foreground register to white */
