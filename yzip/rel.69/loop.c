   ...
   ZIPBYT operand;
{
   ...
   case OPGTPT:               /* GET PROPERTY TABLE */
      ...
      break;
   case OPNEXT:
		/* TAA 7/11/89.  Fix totally non-working implementation--
			always returned 0 on first call, because there's no
			property 0 (should just do firstprp, and return that);
			lost on subsequent calls because it didn't do nxtprp
			before returning. */				
		if (argblk[2] == 0)
			temp = firstprp(argblk[1]);
		else {
	      temp = find_prop( argblk[1], (ZIPBYT)argblk[2], &id );
